...
/**
 * Servicio de Cajas (Puntos de Venta) - Estilo Funcional
 */

/**
 * Crear una nueva caja
 */
export const createCaja = async (tenantId: number, data: CreateCajaDTO): Promise<CajaResponseDTO> => {
 ...
 */
export const getCajasByTenant = async (tenantId: number, includeInactive = false): Promise<CajaResponseDTO[]> => {
 ...
 */
export const getCajaByIdAndTenant = async (cajaId: number, tenantId: number): Promise<CajaResponseDTO | null> => {
 ...
 * Actualizar una caja
 */
export const updateCajaByIdAndTenant = async (
  cajaId: number,
  tenantId: number,
  data: UpdateCajaDTO
  ...
  return getCajaByIdAndTenant(cajaId, tenantId);
};

/**
 * Eliminar (soft delete) una caja
 */
export const deleteCajaByIdAndTenant = async (cajaId: number, tenantId: number): Promise<boolean> => {
